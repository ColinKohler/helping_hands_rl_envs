...
class CovidTestEnv(PyBulletEnv):
    ...
    def resetBoxs(self):
        ...
        T_range = (self.workspace_padding - (np.sin(self.perterb_angel) *
                                             ((self.workspace[0, 1] - self.workspace[
                                                 0, 0]) / 2 - self.workspace_padding)))
        ...
        self.tube_pos_candidate[:, :2] = self.flips[self.flip].dot(
            self.R_perterb_90.dot(self.tube_pos_candidate_o[:, :2].T)).T
        self.swab_pos_candidate[:, :2] = self.flips[self.flip].dot(
            self.R_perterb_90.dot(self.swab_pos_candidate_o[:, :2].T)).T
    ...
    def reset(self):
        ...
        while True:
            ...
            self.resetBoxs()
            try:
                tube_rot = 1.57 + np.random.uniform(-np.pi / 6,
                                                    np.pi / 6) + self.R_angel_after_flip + np.random.randint(2) * np.pi
                for i in range(3):
        ...
        if num_on_table_tube == 1 and num_on_table_swab == 1 and self.used_tube is None:
            for obj in on_table_obj:
                ...
                if obj.object_type_id == constants.TEST_TUBE:
                    ...
                    used_tube_placing_pose[:2] += self.flips[self.flip].dot(
                        self.R_perterb_90.dot(np.array([x_offset, y_offset])))
...
if __name__ == '__main__':
    ...
    env_config = {'workspace': workspace, 'object_init_space': object_init_space, 'max_steps': 10, 'obs_size': 128,
                  ...
                  'random_orientation': True, 'reward_type': 'step_left', 'simulate_grasp': True,
                  'perfect_grasp': False,
                  'object_scale_range': (0.6, 0.6), 'robot': 'kuka', 'object_init_space_check': 'point',
                  'physics_mode': 'slow'}
