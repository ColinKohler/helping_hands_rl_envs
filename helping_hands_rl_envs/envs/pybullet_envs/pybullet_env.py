...
class PyBulletEnv(BaseEnv):
  ...
  '''
    ...
    def initialize(self):
        ...
        pb.changeDynamics(self.table_id, -1, linearDamping=0.04, angularDamping=0.04, restitution=0,
        ...
        if motion_primative == constants.PICK_PRIMATIVE:
            if self.robot.holding_obj is None:
                ...
                self.robot.pick(pos, rot_q, self.pick_pre_offset, dynamic=self.dynamic,
                                ...
                                top_down_approach=self.pick_top_down_approach)
            ...
            if self.robot.holding_obj is not None:
                ...
                self.robot.place(pos, rot_q, self.place_pre_offset,
                                 ...
                                 top_down_approach=self.place_top_down_approach)
        ...
        for i in range(num_shapes):
            ...
            for j in range(100):
                ...
                if sample_range:
                    ...
                    position = [(x_extents - border_padding) * npr.random_sample() + self.getValidSpace()[0][
                        0] + border_padding / 2,
                                (y_extents - border_padding) * npr.random_sample() + self.getValidSpace()[1][
                                    0] + border_padding / 2]
                ...
                if self.pos_candidate is not None:
                    ...
                    if not (self.getValidSpace()[0][0] + border_padding / 2 < position[0] < self.getValidSpace()[0][
                        1] - border_padding / 2 and
                            self.getValidSpace()[1][0] + border_padding / 2 < position[1] < self.getValidSpace()[1][
                                1] - border_padding / 2):
                        continue

                if existing_positions_copy:
                    distances = np.array(
                        list(map(lambda p: np.linalg.norm(np.array(p) - position), existing_positions_copy)))
    ...
    def _getDefaultBoarderPadding(self, shape_type):
        if shape_type in (
        constants.CUBE, constants.TRIANGLE, constants.RANDOM, constants.CYLINDER, constants.RANDOM_BLOCK,
    ...
    def _generateShapes(self, shape_type=0, num_shapes=1, scale=None, pos=None, rot=None, alpha=None, colors=None,
        ...
        for position, orientation in zip(valid_positions, orientations):
            ...
            if shape_type == constants.CUBE:
                ...
                handle = pb_obj_generation.generateGraspNetObject(position, orientation, scale, model_id, alpha, colors)

            else:
                raise NotImplementedError
            if self.physic_mode == 'slow':
                pb.changeDynamics(handle.object_id, -1, linearDamping=0.04, angularDamping=0.04, restitution=0,
                                  contactStiffness=3000, contactDamping=100)
    ...
    def _getNumTopBlock(self, blocks=None):
        ...
        for obj in blocks:
            ...
            for cluster in cluster_pos:
                if np.allclose(block_position[:-1], cluster,
                               atol=self.block_original_size * self.block_scale_range[0] * 2 / 3):
